...
struct HelpView: View {
    ...
    private let topics: [(String, String)] = [
        ...
        ("VoiceOver actions", "On dog, customer, staff and message rows, use the Actions rotor for relevant shortcuts. Activate a row for its profile. Destructive decisions ask for confirmation. In Premises, each grid square announces its coordinate and all its contents, or Empty. Use its Add Item, Move or Remove actions, or activate it for details. To move an item, choose Move and activate its destination square. No dragging is required."),
        ("Building and moving", "Choose Outdoor grounds, Room one or Room two in Premises. Both starter rooms have their own grid and Area Options. Activate a square such as A1 to inspect it or choose an item to place there. Floors and furniture can share a square; its description includes both. Fences belong on the boundary; gates replace panels. Area Options offers room purpose, flooring and boundary work with the actual total cost. Close the site before moving or removing equipment."),
